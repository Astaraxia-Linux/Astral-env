...
EnvConfig load_env_config(const std::filesystem::path& lock_path, const std::filesystem::path& stars_path) {
    ...
    for (const auto& entry : lock.entries) {
        // AstralSource entries install to the host system, not the store —
        // they don't contribute a store path to the environment activation.
        if (entry.build_kind != lock::BuildKind::AstralSource && !entry.store_path.empty()) {
    ...
    if (std::filesystem::exists(stars_path)) {
        ...
        while (std::getline(iss, line)) {
            ...
            if      (line.find("$ENV.Metadata") != std::string::npos) { in_metadata = true;  in_vars = false; in_shell = false; continue; }
            else if (line.find("$ENV.Vars")     != std::string::npos) { in_metadata = false; in_vars = true;  in_shell = false; continue; }
            else if (line.find("$ENV.Shell")    != std::string::npos) { in_metadata = false; in_vars = false; in_shell = true;  continue; }
            else if (line == "};")                                     { in_metadata = false; in_vars = false; in_shell = false; continue; }
            ...
            if (eq_pos != std::string::npos) {
                ...
                if      (in_metadata && key == "Name") config.name = value;
                else if (in_vars)                       config.vars.emplace_back(key, value);
    ...
    if (config.name.empty()) config.name = "unknown";
    return config;
}
...
void enter_shell(
    ...
    if      (shell_name == "bash") exec_args = {"--rcfile", script_path.string()};
    else if (shell_name == "zsh")  exec_args = {"--rcs",    script_path.string()};
    else                            env_additions = {{"ENV", script_path.string()}};
